...
def main():
            ...
            if st.button(prepare_arabic_text("🔄 تحديث البيانات من البنك المركزي"), use_container_width=True, type="primary"):
                with st.spinner(prepare_arabic_text("جاري جلب أحدث البيانات... قد يستغرق الأمر دقيقة.")):
                    try:
                        ...
                        st.error(
                            prepare_arabic_text("⚠️ حدث خطأ أثناء محاولة تحديث البيانات. يرجى التأكد من اتصالك بالإنترنت والمحاولة مرة أخرى."),
                            icon="⚠️"
                        )
                st.rerun()
    ...
    if calculate_button_main and selected_tenor_main is not None:
        try:
            ...
            with results_placeholder_main.container(border=True):
                st.error(
                    prepare_arabic_text("حدث خطأ غير متوقع أثناء الحساب. يرجى التحقق من المدخلات."),
                    icon="🚨"
                )
    ...
    with st.expander(prepare_arabic_text("💡 شرح ومساعدة (أسئلة شائعة)")):
        ...
        # --- **IMPROVEMENT**: Updated the description for the fee calculator ---
