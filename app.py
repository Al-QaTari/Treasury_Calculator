...
def main():
    ...
    with top_col1:
        with st.container(border=True):
            ...
            if (
                not data_df.empty
                and "البيانات الأولية" not in last_update
            ):
                day_names_en_ar = {
    ...
    with col_form_main:
        with st.container(border=True):
            ...
            tax_rate_main = st.number_input(
                ...
                0.0, 100.0, C.DEFAULT_TAX_RATE_PERCENT, step=0.5, format="%.1f"
            )
        ...
        try:
            if data_df.empty:
                ...
                if yield_rate is not None:
                    ...
                    with results_placeholder_main.container(border=True):
                        st.subheader(
                            prepare_arabic_text(f"✨ ملخص استثمارك لأجل {selected_tenor_main} يوم"),
                            anchor=False,
                        )
                        st.markdown(
    ...
    with col_secondary_form:
        with st.container(border=True):
            ...
            face_value_secondary = st.number_input(
                ...
                min_value=25000.0, value=25000.0, step=25000.0, key="secondary_face_value"
            )
            original_yield_secondary = st.number_input(
                prepare_arabic_text("عائد الشراء الأصلي (%)"),
                min_value=1.0, value=29.0, step=0.1, key="secondary_original_yield", format="%.3f"
            )
            original_tenor_secondary = st.selectbox(
                prepare_arabic_text("أجل الإذن الأصلي (بالأيام)"),
                options, key="secondary_tenor"
            )
            tax_rate_secondary = st.number_input(
                prepare_arabic_text("نسبة الضريبة على الأرباح (%)"),
                0.0, 100.0, C.DEFAULT_TAX_RATE_PERCENT, step=0.5, format="%.1f", key="secondary_tax"
            )
            ...
            early_sale_days_secondary = st.number_input(
                ...
                min_value=1, value=min(60, max_holding_days), max_value=max_holding_days, step=1
            )
            secondary_market_yield = st.number_input(
                prepare_arabic_text("العائد السائد في السوق للمشتري (%)"),
                min_value=1.0, value=30.0, step=0.1, format="%.3f"
            )
            calc_secondary_sale_button = st.button(
                prepare_arabic_text("حلل سعر البيع الثانوي"),
                use_container_width=True, type="primary", key="secondary_calc"
            )
    ...
    if calc_secondary_sale_button:
        try:
            results = analyze_secondary_sale(
                face_value_secondary, original_yield_secondary, original_tenor_secondary,
                early_sale_days_secondary, secondary_market_yield, tax_rate_secondary
            )
            with secondary_results_placeholder.container(border=True):
                ...
                if results.get("error"):
                    ...
                    if results["net_profit"] >= 0:
                        st.success(f"البيع الآن يعتبر مربحًا. ستحقق ربحًا صافيًا قدره {results['net_profit']:,.2f} جنيه.", icon="✅")
                    else:
                        st.warning(f"البيع الآن سيحقق خسارة. ستبلغ خسارتك الصافية {abs(results['net_profit']):,.2f} جنيه.", icon="⚠️")
                    st.markdown("---")
    ...
    if not historical_df.empty:
        ...
        if selected_tenors:
            ...
            fig = px.line(
                ...
                title=prepare_arabic_text("التغير في متوسط العائد المرجح لأذون الخزانة"),
            )
            fig.update_layout(
    ...
    with st.expander(prepare_arabic_text("💡 شرح ومساعدة (أسئلة شائعة)")):
        ...
        with fee_col1:
            total_face_value = st.number_input(
                ...
                min_value=25000.0, value=100000.0, step=25000.0, key="fee_calc_total"
            )
        with fee_col2:
            fee_percentage = st.number_input(
                prepare_arabic_text("نسبة رسوم الحفظ السنوية (%)"),
                min_value=0.0, value=0.10, step=0.01, format="%.2f", key="fee_calc_perc"
            )
