...
def main():
    ...
    # --- Secondary Market Sale Calculator (Restored) ---
    st.divider()
    ...
    # --- Help Section (Updated with Fee Calculator) ---
    st.divider()
    with st.expander(prepare_arabic_text("💡 شرح ومساعدة (أسئلة شائعة)")):
        ...
        # --- MODIFIED: Fee calculator is now inside the help section ---
        st.markdown("---")
        st.subheader(prepare_arabic_text("حاسبة رسوم أمين الحفظ"))
        st.markdown(prepare_arabic_text("توجد رسوم إدارية بسيطة على إجمالي استثماراتك. تختلف هذه النسبة من بنك لآخر (مثلاً 0.1% في البنك الأهلي). استخدم الحاسبة التالية لمعرفة الرسوم التقريبية:"))
