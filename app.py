...
def main():
    ...
    with col_form_main:
        with st.container(border=True):
            ...
            investment_amount_main = st.number_input(prepare_arabic_text("المبلغ المستهدف في نهاية المدة (القيمة الإسمية)"), min_value=1000.0, value=100000.0, step=1000.0)
        ...
        with secondary_results_placeholder.container(border=True):
            ...
            if results.get("error"):
                ...
                if results["net_profit"] >= 0:
                    st.success(prepare_arabic_text(f"البيع الآن يعتبر مربحًا. ستحقق ربحًا صافيًا قدره {results['net_profit']:,.2f} جنيه."), icon="✅")
                else:
                    st.warning(prepare_arabic_text(f"البيع الآن سيحقق خسارة. ستبلغ خسارتك الصافية {abs(results['net_profit']):,.2f} جنيه."), icon="⚠️")
